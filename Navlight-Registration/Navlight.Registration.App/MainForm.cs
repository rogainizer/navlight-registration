...
public sealed class MainForm : Form
{
    ...
    private async void SaveButton_Click(object? sender, EventArgs e)
    {
        ...
        ToggleBusyState(true, "Saving registration...");

        try
        {
            await _repository.SaveRegistrationAsync(_currentTeam);
            AppNavigation.LastSavedRegistrationTeamId = _currentTeam.TeamId;
            ResetForNextRegistration();
            SetStatus("Registration saved. Ready for next team.");
        }
        catch (Exception ex)
        {
    ...
    private async Task LoadSelectedTeamAsync()
    {
        ...
        try
        {
            _currentTeam = await _repository.GetTeamRegistrationAsync(selectedTeam.TeamId);
            await BindTeamAsync();
    ...
    private async Task ReloadCurrentTeamAsync()
    {
        if (_currentTeam is null)
        {
            return;
        ...
        _currentTeam = await _repository.GetTeamRegistrationAsync(_currentTeam.TeamId);
        await BindTeamAsync();
    }
